...
public class PictureEditView extends FrameLayout implements Runnable, ScaleGestureDetector.OnScaleGestureListener,
    ...
    public EditPresenter getEditPresenter(){
        return mPicPresenter;
    }

    public void setEditPresenter(EditPresenter mEditPresenter){
        if(mEditPresenter.mBackStickers.size()>0){
            mPicPresenter.mBackStickers.clear();
            mPicPresenter.mBackStickers.addAll(mEditPresenter.mBackStickers);
        }
        if(mEditPresenter.mDoodleList.size()>0){
            mPicPresenter.mDoodleList.clear();
            mPicPresenter.mDoodleList.addAll(mEditPresenter.mDoodleList);
        }
        if(mEditPresenter.mMosaicList.size()>0){
            mPicPresenter.mMosaicList.clear();
            mPicPresenter.mMosaicList.addAll(mEditPresenter.mMosaicList);
        }
        if(mEditPresenter.mHistoryList.size()>0){
            mPicPresenter.mHistoryList.clear();
            mPicPresenter.mHistoryList.addAll(mEditPresenter.mHistoryList);
        }
     ...
     * @param paintPaths
     */
    public void addHistoryDoodleList(List<PaintPath> paintPaths) {
        mPicPresenter.addHistoryDoodleList(paintPaths);
        invalidate();
    }
     ...
     * @return
     */
    public List<PaintPath> getHistoryDoodleList() {
        return mPicPresenter.getHistoryDoodleList();
    }


    public void clearAllDoodle() {
        mPicPresenter.clearAllDoodle();
        invalidate();
    }
