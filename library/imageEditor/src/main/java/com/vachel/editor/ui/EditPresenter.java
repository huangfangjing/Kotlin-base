...
public class EditPresenter implements Serializable {
    private Bitmap mDoodleLayer, mMosaicLayer;  //mDoodleLayer画笔， mMosaicLayer马赛克
   // 底图区域
    ...
    public final List<ISticker> mBackStickers = new ArrayList<>();

    // 涂鸦路径
    public final List<PaintPath> mDoodleList = new ArrayList<>();
    //马赛克路径
    public final List<PaintPath> mMosaicList = new ArrayList<>();
    // 需保证与mDoodleList mHistoryList的数据一致性，用于undo操作
    public final List<PaintPath> mHistoryList = new ArrayList<>();
    ...
    public void addHistoryDoodleList(List<PaintPath> paintPaths){
        mHistoryList.clear();
        mDoodleList.clear();
        mHistoryList.addAll(paintPaths);
        mDoodleList.addAll(paintPaths);
    }
...
//            mClipFrame.intersect(mFrame);
    ...
    public void release() {
        //使用Glide会自动处理好Bitmap回收，无需手动回收否则报错
