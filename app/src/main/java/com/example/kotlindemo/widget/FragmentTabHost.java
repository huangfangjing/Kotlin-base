...
public class FragmentTabHost extends TabHost implements
    ...
    private void ensureHierarchy(Context context,int marginLeft,int marginRight) {
        // If owner hasn't made its own view hierarchy, then as a convenience
        // we will construct a standard one here.
        if (findViewById(android.R.id.tabs) == null) {
            LinearLayout ll = new LinearLayout(context);
            ...
            LinearLayout.LayoutParams llParams = new LinearLayout.LayoutParams(
                    ViewGroup.LayoutParams.MATCH_PARENT,
                    ViewGroup.LayoutParams.WRAP_CONTENT, 0);
            llParams.setMargins(marginLeft,0,marginRight,0);//设置TabWidget的边距
            ll.addView(tw, llParams);
    ...
    public void setup(Context context, FragmentManager manager, int containerId,int marginLeft,int marginRight) {
        ensureHierarchy(context,marginLeft,marginRight); // Ensure views required by super.setup()
        super.setup();
        mContext = context;
        mFragmentManager = manager;
        mContainerId = containerId;
